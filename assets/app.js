...
async function loadEvents() {
    ...
    try {
        ...
        els.calendarGrid.innerHTML = `<div class="empty-state wide">${message}</div>`;
        els.monthList.innerHTML = `<div class="empty-state">${message}</div>`;
    } finally {
        setLoading(false);
    }
}
...
function setLoading(isLoading) {
    ...
    if (isLoading) {
        els.calendarGrid.innerHTML = '<div class="empty-state wide">Ładowanie wydarzeń...</div>';
        els.monthList.innerHTML = '<div class="empty-state">Ładowanie wydarzeń...</div>';
    }
}
...
function renderMonthList() {
    ...
    if (monthEvents.length === 0) {
        els.monthList.innerHTML = '<div class="empty-state">Brak wydarzeń w tym miesiącu.</div>';
        return;
    }

    const groups = new Map();
    monthEvents.forEach((event) => {
        const key = dateKey(mysqlToDate(event.start_at));
    ...
    groups.forEach((events, key) => {
        ...
        section.className = 'month-list-day';
        const heading = document.createElement('h3');
        heading.textContent = mysqlToDate(`${key} 12:00:00`).toLocaleDateString('pl-PL', {
            weekday: 'long',
            day: '2-digit',
            month: 'long',
        });
        ...
        events.forEach((event) => section.appendChild(monthListItem(event)));
        els.monthList.appendChild(section);
    });
}

function monthListItem(event) {
    const item = document.createElement('article');
    item.className = `month-list-item type-border-${event.type}`;
    if (event.status === 'done') item.classList.add('is-done');

    const meta = [
        formatTime(event.start_at),
        typeLabels[event.type],
        event.location || '',
        event.can_edit ? '' : `od ${event.owner_name || 'znajomego'}`,
    ].filter(Boolean).map(escapeHtml).join(' · ');

    item.innerHTML = `
        <div>
            <span class="agenda-time">${meta}</span>
            <strong>${escapeHtml(event.title)}</strong>
            ${event.prep_note ? `<span>${escapeHtml(event.prep_note)}</span>` : ''}
        </div>
        <div class="item-actions">
            <button class="button muted small" data-action="open" type="button">Szczegóły</button>
            ${event.can_edit ? `<button class="button small ${event.status === 'done' ? 'muted' : 'primary'}" data-action="toggle" type="button">${event.status === 'done' ? 'Przywróć' : 'Zrobione'}</button>` : ''}
        </div>
    `;

    item.querySelector('[data-action="open"]').addEventListener('click', () => openExistingEvent(event));
    const toggle = item.querySelector('[data-action="toggle"]');
    if (toggle) {
        toggle.addEventListener('click', () => toggleEventStatus(event).catch((error) => {
            alert(error.message);
        }));
    }
...
function updateView() {
    ...
    els.calendarGrid.hidden = isList;
    $('.weekdays').hidden = isList;
    els.monthList.hidden = !isList;
...
async function toggleEventStatus(event) {
    ...
    await requestJson(`api/events.php?id=${encodeURIComponent(event.source_id || event.id)}`, {
        method: 'PUT',
        body: JSON.stringify({ status: event.status === 'done' ? 'planned' : 'done' }),
    });
    await loadEvents();
}
